"""Aggregates Rhode Island-located listings from every taildraggers.com
...
ones whose location ends in ", RI".
...
Pennsylvania repos' approach, with the state filter swapped to "RI".
"""
...
PAGE_TITLE = "Taildragger Ads in Rhode Island"
STATE_ABBR = "RI"
...
_RHODE_ISLAND_LOCATION_RE = re.compile(r",\s*RI\s*$", re.IGNORECASE)


def _is_rhode_island(location: str) -> bool:
    return bool(_RHODE_ISLAND_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        rhode_island_rows = [row for row in rows if _is_rhode_island(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(rhode_island_rows)} in Rhode Island")
        all_listings.extend(rhode_island_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Rhode Island listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Rhode Island listings: {len(listings)}")
