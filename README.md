...
Daily aggregator of Rhode Island-located taildragger classified
...
[Pennsylvania](https://github.com/taildraggers/pennsylvania) repos'
approach, with the state filter swapped to "RI".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, RI` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Rhode Island"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/rhode-island/`
...
```html
<iframe
  src="https://taildraggers.github.io/rhode-island/"
  title="Taildragger Ads in Rhode Island"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_RHODE_ISLAND_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
